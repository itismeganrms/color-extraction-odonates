...
def main(args):
   ...
   # If super resolution is enabled, divide the coordinates of the predictions by 4 and areas by 4**2
    if args.super_resolution is True and scale_factor > 1:
        ...
        with open(f"/h/jquinto/Mask-RCNN/runs/predict/{exp_name}/result.json", "r") as f:
        ...
        with open(f"/h/jquinto/Mask-RCNN/runs/predict/{exp_name}/result.json", "w") as f:
    ...
    for det_type in ["bbox", "segm"]:
        evaluate(
            ...
            result_json_path=f"/h/jquinto/Mask-RCNN/runs/predict/{exp_name}/result.json",
            out_dir=f"/h/jquinto/Mask-RCNN/runs/predict/{exp_name}",
            type=det_type,
            classwise=True,
            max_detections=1000,
            areas = areas
