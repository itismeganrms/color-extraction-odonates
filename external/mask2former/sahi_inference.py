...
# from sahi.models.detectron2_mask2former import Detectron2DetectionModel
...
def main(args):
    ...
    if os.path.exists(f"/h/jquinto/Mask2Former/runs/predict/{exp_name}"):
    ...
    if args.super_resolution is True and scale_factor > 1:
        ...
        with open(f"/h/jquinto/Mask2Former/runs/predict/{exp_name}/result.json", "r") as f:
        ...
        with open(f"/h/jquinto/Mask2Former/runs/predict/{exp_name}/result.json", "w") as f:
    ...
    for det_type in ["bbox", "segm"]:
        evaluate(
            ...
            result_json_path=f"/h/jquinto/Mask2Former/runs/predict/{exp_name}/result.json",
            out_dir=f"/h/jquinto/Mask2Former/runs/predict/{exp_name}",
            type=det_type,
            classwise=True,
            max_detections=1000,
            areas = areas
